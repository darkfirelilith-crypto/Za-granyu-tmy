...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const sheet = await db.vtmSheet.findUnique({ where: { id } });
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!body.force && body.baseUpdatedAt) {
    ...
    if (!isNaN(base) && serverTs - base > 3000) {
      ...
      return NextResponse.json(
        {
          error: "Лист изменён в другом окне",
  ...
  const updated = await db.vtmSheet.update({
    where: { id },
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await db.vtmSheet.delete({ where: { id } });
