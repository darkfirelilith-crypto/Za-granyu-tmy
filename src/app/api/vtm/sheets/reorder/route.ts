...
/** POST /api/vtm/sheets/reorder — сохранить ручной порядок листов архива.
...
export async function POST(req: NextRequest) {
  ...
  // проверяем владение всеми листами
  const owned = await db.vtmSheet.findMany({
  ...
  await db.$transaction(
    ids.map((id, i) =>
      db.vtmSheet.update({ where: { id }, data: { sortOrder: i + 1 }, select: { id: true } })
    )
  );
